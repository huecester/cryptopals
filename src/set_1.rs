...
fn challenge_4() -> Result<()> {
	let lines = read_to_string("res/4.txt")?;
	let mut guesses: Vec<(Data, i32)> = lines.split_ascii_whitespace()
		.map(|line| {
		...
		.collect::<Result<_>>()?;
	guesses.sort_by_key(|guess| -guess.1);
...
fn challenge_6() -> Result<()> {
	...
	let raw_data = read_to_string("res/6.txt")?.replace('\n', "");
...
fn challenge_7() -> Result<()> {
	let ciphertext = Data::from_b64(&read_to_string("res/7.txt")?.replace('\n', ""))?;
...
fn challenge_8() -> Result<()> {
	let file = read_to_string("res/8.txt")?;
	let data: Vec<_> = file
		.trim()
		.split('\n')
		.map(|line| Ok(Data::from_hex(line.trim())?))
		.collect::<Result<_>>()?;
	assert_eq!(

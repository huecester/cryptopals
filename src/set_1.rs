...
#[ignore = "slow"]
fn challenge_4() {
	let lines = read_to_string("res/4.txt").unwrap();
...
fn challenge_6() {
	...
	let raw_data = read_to_string("res/6.txt").unwrap().replace('\n', "");
...
#[test]
fn challenge_7() {
	let ciphertext = Data::from_b64(&read_to_string("res/7.txt").unwrap().replace('\n', ""));
...
#[test]
fn challenge_8() {
	let file = read_to_string("res/8.txt").unwrap();
	let data: Vec<_> = file.trim().split('\n').map(|line| Data::from_hex(line.trim())).collect();
	assert_eq!(
